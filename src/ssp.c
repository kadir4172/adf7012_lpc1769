...
void SSP0Init( void )
{
  ...
  LPC_SC->PCONP |= (0x1<<21); //ssp0 icin power ve clock enable edildi

  /* Further divider is needed on SSP0 clock. Using default divided by 4 */
  LPC_SC->PCLKSEL1 &= ~(0x3<<10); //72Mhz /4 secildi
  ...
  LPC_GPIO0->FIODIR |= (0x1<<15);		/* P0.15 defined Outputs */
  LPC_GPIO0->FIODIR |= (0x1<<18);		/* P0.18 defined Outputs */
  LPC_GPIO0->FIODIR &= ~(0x1<<17);		/* P0.17 defined Inputs */
  ...
  LPC_SSP0->CR0 = 0x0707; //8bit transfer, SPI format, bitler arasinda bus clock low da kalsin, peripharal clock/16 clock rate
...
void SSP1Init( void )
{
  ...
  LPC_SC->PCLKSEL0 &= ~(0x3<<20); //72Mhz/4 pclk
  ...
  /* Set DSS data to 8-bit, Frame format SPI, CPOL = 0, CPHA = 0, and SCR is 15 */
  LPC_SSP1->CR0 = 0x0707; //8bit transfer, SPI format, bitler arasinda bus clock low da kalsin, peripharal clock/16 clock rate

  /* SSPCPSR clock prescale register, master mode, minimum divisor is 0x02 */
  LPC_SSP1->CPSR = 0x40; //prescale = 64 yani  18Mhz/(8*64) yaklasik 35 kHz Spi clock rate
